...
A --> E["导航栏<br/>src/components/navbar.tsx"]
A --> F["鼠标跟随装饰<br/>src/components/mouse-follower.tsx"]
A --> G["首页<br/>src/app/page.tsx"]
A --> H["游戏会话页<br/>src/app/game/[sessionId]/page.tsx"]
I["Next 配置<br/>next.config.ts"] --> J["API 代理重写<br/>/api/v1/* -> 后端"]
K["shadcn 配置<br/>components.json"] --> L["路径别名与样式入口"]
M["TypeScript 配置<br/>tsconfig.json"] --> N["@/* 路径映射"]
```
...
- [context.tsx:1-55](file://frontend/src/lib/i18n/context.tsx#L1-L55)
...
- 国际化上下文：LanguageProvider 维护 locale 与 t(key)，持久化到 localStorage，默认回退英文。
...
### 状态管理与国际化
- LanguageProvider：维护 locale 与 setLocale，t(key) 查找翻译，默认回退英文，localStorage 持久化。
- Providers：在 RootLayout 中包裹 LanguageProvider，确保全应用可用。
- LanguageSwitcher：下拉切换 en/zh-CN/zh-TW，点击外部关闭。

```mermaid
flowchart TD
Start(["应用启动"]) --> Init["初始化 LanguageProvider"]
Init --> Restore["从 localStorage 恢复 locale"]
Restore --> Render["渲染子组件"]
...
- [context.tsx:1-55](file://frontend/src/lib/i18n/context.tsx#L1-L55)
- [providers.tsx:1-8](file://frontend/src/components/providers.tsx#L1-L8)
- [language-switcher.tsx:1-42](file://frontend/src/components/language-switcher.tsx#L1-L42)
...
- 国际化缺失：
  - 检查 translations 中是否包含对应 key，必要时回退英文。
- 导航异常：
...
## 结论
本项目以 Next.js App Router 为核心，结合 React 19 与 Tailwind v4，构建了清晰的前端架构。UI 层基于 shadcn/ui 实现一致的设计系统，API 层统一封装保障稳定性与可维护性。国际化与主题系统完善，性能与用户体验兼顾。后续可在分页、缓存与更细粒度的状态管理上持续优化。
